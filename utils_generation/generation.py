...
def calZeroAndHiddenStates(model, tokenizer, frame_dict, args):
    ...
    with torch.no_grad():
        ...
        for key, record in zip(frame_dict.keys(), records):
            ...
            if args.cal_zeroshot or args.cal_logits:
                ...
                for idx in range(len(frame)):
                    if "gpt" in mdl_name or "bert" in mdl_name:
                        ...
                        logits = [model(input_ids, labels=w)["logits"] for w in ans_token]
                ...
                if args.cal_zeroshot:
                    ...
                    record["log_probs"] = sum([int(w[0] < w[1]) == l for w, l in zip(log_probs_list, labels)]) / len(
                        frame
                    )
                    record["calibrated"] = getCalibrated(log_probs_list, labels)

                    if args.print_more:
                        print("Finish calculating the zero-shot accuracy for {} data in {}.".format(len(frame), key))
                ...
                if args.cal_logits:
                    ...
                    if args.print_more:
                        print(
                            "Finish generating logits for {} data in {}. Shape of logits is {}.".format(
                                len(frame), key, logits.shape
                            )
                        )
                ...
                for idx in range(len(frame)):
                    ...
                    if "T0" in mdl_name or "unifiedqa" in mdl_name or "t5" in mdl_name:
                        if args.states_location == "encoder":
                            ...
                            hidden_states_paired = [
                                model(input_ids, labels=ans, output_hidden_states=True).decoder_hidden_states
                    ...
                    for i in range(num_labels):
                        ...
                        hidden_states[i].append(
                            np.stack(
                                [toNP(getStatesToken(w, args.token_place)) for w in hidden_states_paired[i]], axis=0
                            )
                        )
            ...
            print("{} s/data. {}".format(round((time.time() - start) / record["population"], 2), record))
...
def getLogProbs(logits, ans_token, mdl_name):
    ...
    if "gpt" in mdl_name:
        return [
            torch.mean(logit.log_softmax(dim=-1)[range(-ans.shape[1] - 1, -1), ans[0]])
        ...
        return [
            torch.mean(logit.log_softmax(dim=-1)[range(ans.shape[1] - 1), ans[0][:-1]])
            for logit, ans in zip(logits, ans_token)
        ]
