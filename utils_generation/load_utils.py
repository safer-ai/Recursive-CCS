...
def loadModel(mdl_name, cache_dir, parallelize):
    ...
    if mdl_name in ["gpt-neo-2.7B", "gpt-j-6B"]:
        ...
        model = AutoModelForSeq2SeqLM.from_pretrained("bigscience/{}".format(mdl_name), cache_dir=cache_dir)
        tokenizer = AutoTokenizer.from_pretrained("bigscience/{}".format(mdl_name), cache_dir=cache_dir)
    elif "unifiedqa" in mdl_name:
        model = T5ForConditionalGeneration.from_pretrained("allenai/" + mdl_name, cache_dir=cache_dir)
        tokenizer = AutoTokenizer.from_pretrained("allenai/" + mdl_name, cache_dir=cache_dir)
    elif "deberta" in mdl_name:
        model = AutoModelForSequenceClassification.from_pretrained("microsoft/{}".format(mdl_name), cache_dir=cache_dir)
        tokenizer = AutoTokenizer.from_pretrained("microsoft/" + mdl_name, cache_dir=cache_dir)
    ...
    print("finish loading model to memory. Now start loading to gpu. parallelize = {}".format(parallelize == True))
    if parallelize:
        model.parallelize()
    else:
        model = model.to("cuda")
            ...
            tmp_lis.append(
                pd.concat(
                    [
                        ...
                        data_list[1][data_list[1][lbl_tag] == lbl][: data_num_lis[i] - prior_size],
                    ],
                    ignore_index=True,
                )
            )
...
def loadFromDatasets(set_name, cache_dir, max_num):
    ...
    raw_data = get_sample_data(set_name, [raw_set[w].to_pandas() for w in token_list], 2 * max_num)
...
def loadDatasets(args, tokenizer):
    ...
    for (set_name, prompt_idx, max_num) in zip(set_list, prompt_idx_list, num_data):
        ...
        if reload == False and os.path.exists(os.path.join(complete_path, "frame.csv")):
            ...
            if args.print_more:
                print("load post-processing {} from {}, length = {}".format(dataset_name_w_num, complete_path, max_num))
            ...
            if (reload == False or reload_set_name == set_name) and os.path.exists(path):
                ...
                if args.print_more:
                    print("load raw {} from {}, length = {}".format(set_name, path, max_num))
            else:
                if args.print_more:
