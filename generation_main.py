...
if __name__ == "__main__":
    ...
    model, tokenizer = loadModel(mdl_name=args.model, cache_dir=args.cache_dir, parallelize=args.parallelize)
    ...
    for prefix in prefix_list:
        ...
        print("Time: {}".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
        print(
            "Consider prefix {}, {} datasets, {} samples in total, and took {} minutes. Generation: {}, Hidden States: {}".format(
                prefix,
                len(frame_dict),
                sum([len(w) for w in frame_dict.values()]),
                round((end - start) / 60, 1),
                args.cal_zeroshot == True,
                args.cal_hiddenstates == True,
            )
        )
        print("---------------------------------------\n\n")
